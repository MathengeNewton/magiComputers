...
import { createPostSchema, updatePostSchema } from '@magicomputers/shared';
...
export class PostsService {
  ...
  async create(tenantId: string, createDto: any) {
    ...
    // Validate destinations belong to the tenant
    if (validated.destinationIds && validated.destinationIds.length > 0) {
      ...
      if (destinations.length !== validated.destinationIds.length) {
        throw new BadRequestException('One or more destinations do not belong to the tenant');
      }
    }
    ...
    if (allMediaIds.size > 0) {
      ...
      if (media.length !== allMediaIds.size) {
        throw new BadRequestException('One or more media files do not belong to the tenant');
      }
    }
