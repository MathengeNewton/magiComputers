...
type ProductsListingProps = {
  ...
  /** When 'shop', breadcrumb and category links use /shop and /shop/[slug]. Default 'products'. */
...
export default function ProductsListing({
  ...
  const base = basePath === 'shop' ? '/shop' : '/products';
  const baseLabel = basePath === 'shop' ? 'Shop' : 'Products';
    ...
    <div className="min-h-screen bg-shop-bg">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <nav className="mb-6 text-sm text-shop-muted">
          ...
          <Link href={base} className="hover:text-shop-fg transition-colors">{baseLabel}</Link>
          {categorySlug && (
            <>
              <span className="mx-2">›</span>
